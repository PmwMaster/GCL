  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatCard
          icon={<FolderKanban className="h-5 w-5 text-primary-600" />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <h2 className="text-sm font-semibold text-gray-700 mb-4">Projetos por Tipo de Serviço</h2>
          <div className="space-y-3">
            {Object.entries(stats.projetosPorTipo).map(([tipo, count]) => (
              <div key={tipo} className="flex items-center justify-between">
                <span className="text-sm text-gray-600">{TIPO_SERVICO_LABELS[tipo as keyof typeof TIPO_SERVICO_LABELS]}</span>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <h2 className="text-sm font-semibold text-gray-700 mb-4">Carga de Tarefas por Sócio</h2>
          <div className="space-y-3">
            {stats.cargaPorSocio.map(s => (
              <div key={s.nome} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-medium text-gray-900 w-6 text-right">{s.tarefas}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5 lg:col-span-2">
          <h2 className="text-sm font-semibold text-gray-700 mb-4">Próximos Prazos</h2>
              ...
              <tbody>
                {stats.projetosProximos.map(p => (
                  <tr key={p.id} className="border-b last:border-0">
                    <td className="py-2.5 font-medium text-gray-900">{p.nome}</td>
                    <td className="py-2.5 text-gray-600">{(p.clients as unknown as { nome: string })?.nome ?? '-'}</td>
                    <td className="py-2.5 text-gray-600">{TIPO_SERVICO_LABELS[p.tipo_servico]}</td>
                    <td className="py-2.5 text-gray-600">{p.prazo_entrega ? new Date(p.prazo_entrega).toLocaleDateString('pt-BR') : '-'}</td>
                  </tr>
                ))}
